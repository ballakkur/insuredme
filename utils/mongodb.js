...
exports.connect = (callback) => {
    ...
    MongoClient.connect( process.env.url,process.env.dbName,{ useUnifiedTopology: true }, (err, connection) => {
        console.error(`MongoDB connection url ${process.env.url}`, process.env.dbName);
        if (err) {
        ...
        // console.info(`MongoDB connection successfully established `,state.db);
        // console.info(`MongoDB connection successfully established `,state.db.collection('userList') );
        return callback();
    })
}
